...
export const CartItem = () => {
  const { productData, incrementQuantity, deleteFromCart, decrementQantity } = useCartStore();
  ...
  return (
    <div className="w-2/3 pr-10">
      ...
      <div>
         ...
         productData.map((item, key)=>{
          return <div key={item._id} className="flex items-center justify-between gap-6 mt-6">
            <div className="flex items-center gap-2">
              <CloseIcon onClick={()=> deleteFromCart(item._id)}  className="text-xl text-gray-600 hover:text-red-600 cursor-pointer duration-300"/>
            ...
            <div className='w-52 flex items-center justify-between text-gray-500 gap-4 border p-3'>
                  ...
                  <div className='flex items-center gap-4 text-sm font-semibold'>
                    <button onClick={()=> decrementQantity({
                      _id: item._id,
                      title: item.title,
                      image: item.image,
                      price: item.price,
        ...
        <button onClick={()=> (resetCart())} className='text-xl font-bold  bg-red-500 text-white py-1 ml-6 px-6 mt-8'>Reset Cart</button>
